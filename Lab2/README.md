...
Σύμφωνα με την συνάρτηση κόστους που κατασκευάσαμε, για να μεγιστοποιήσουμε τον
λόγο απόδοσης προς κόστος, επιλέγουμε την πιο απλή υλοποίηση για κάθε παράμετρο.
Συγκεκριμένα, επιλέγουμε μέγεθος γραμμής 32B, 1MB 8-way L2, 32KB 2-way D-Cache
και 16KB 2-way I-Cache. Την μεγαλύτερη διαφορά στο κόστος την εντοπίζουμε στην
αλλαγή των παραμέτρων της L2 και του μεγέθους γραμμής. Αν αυξήσουμε το μέγεθος ή
το associativity της L1, η σχετική απόδοση δεν μεταβάλλεται σημαντικά διότι
καθώς αυξάνεται το κόστος, βελτιώνεται και η απόδοση. Θα περιμέναμε τα τελικά
δεδομένα να εμφανίζουν τοπικά ελάχιστα και μέγιστα αλλά με την συγκεκριμένη
συνάρτηση δεν παρατηρήσαμε τέτοια φαινόμενα. Συμπερασματικά, για να σχεδιάσουμε
ένα αποδοτικό σε σχέση με το κόστος σύστημα, πρέπει να συμβιβαστούμε με μία
σημαντικά μειωμένη μέγιστη απόδοση, καθώς η αύξηση των παράμετρων της κρυφής
μνήμης είναι αρκετά δαπανηρή.
